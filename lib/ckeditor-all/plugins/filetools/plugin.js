...
;(function() {
    CKEDITOR.plugins.add('filetools', {
        // lang:
        //   "az,bg,ca,cs,da,de,de-ch,en,en-au,eo,es,es-mx,et,eu,fa,fr,gl,hr,hu,id,it,ja,km,ko,ku,lv,nb,nl,no,oc,pl,pt,pt-br,ro,ru,sk,sq,sr,sr-latn,sv,tr,ug,uk,zh,zh-cn", // %REMOVE_LINE_CORE%

        lang: 'en',
        beforeInit: function(editor) {
            // window.FT_EDITOR = editor; // TEST
            // console.log("FT:beforeInit editor = ", editor);
            /**
            ...
            const resolveUpload = (evt, res) => {
                console.log('PLG:resolveUpload evt = ', evt)
                console.log('PLG:resolveUpload  res = ', res)
                console.log('PLG:resolveUpload evt.data.fileLoader = ', evt.data.fileLoader)
                console.log('PLG:resolveUpload evt.data.fileLoader.isFinished = ', evt.data.fileLoader.isFinished)
                // evt.sender.responseData = {
                //   url: 'https://127.0.0.1:3000/media/00/ck/test/coffee-1-100x100.jpg',
                //   message: 'djfghdsfjgh'
                // }
                // evt.data.fileLoader.responseData = {
                //   url: "https://127.0.0.1:3000/media/00/ck/test/coffee-1-100x100.jpg",
                // };
                ...
                evt.data.fileLoader.changeStatus('uploaded') // or: loaded
                evt.data.fileLoader.isFinished('loaded')
            }
            ...
            editor.on(
                'fileUploadResponse',
                function(evt) {
                    console.log('PLG:fileUploadResponse evt = ', evt)
    ...
    UploadRepository.prototype = {
         ...
         */
        isFinished: function() {
            console.log('PLG:isFinished 1 this.loaders.length = ', this.loaders.length)
    ...
    FileLoader.prototype = {
        ...
        upload: function(url, additionalRequestParameters) {
            console.log('PL:upload url, additionalRequestParameters = ', url, additionalRequestParameters)
        ...
        changeStatus: function(newStatus) {
            ...
            if (newStatus == 'error' || newStatus == 'abort' || newStatus == 'loaded' || newStatus == 'uploaded') {
                this.abort = function() {}
            }
         ...
         */
        isFinished: function() {
            const match = !!this.status.match(/^(?:loaded|uploaded|error|abort)$/)
            console.log('PLG:isFinished 2 this.status = ', this.status)
            console.log('PLG:isFinished 2 match = ', match)
