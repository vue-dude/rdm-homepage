...
CKEDITOR.resourceManager.prototype = {
  ...
  add: function (name, definition) {
    ...
    CKEDITOR.fire(
      name + CKEDITOR.tools.capitalize(this.fileName) + "Ready",
      resource
    );
  ...
  getPath: function (name) {
    ...
    return CKEDITOR.getUrl(
      (external && external.dir) || this.basePath + name + "/"
    );
  },
  ...
  getFilePath: function (name) {
    var external = this.externals[name];
    return CKEDITOR.getUrl(
      this.getPath(name) + (external ? external.file : this.fileName + ".js")
    );
  },
